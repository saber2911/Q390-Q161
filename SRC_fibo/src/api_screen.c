...
{
	return hal_scrClrLine(startline, endline);
}
...
{
	hal_scrSetBackLightMode(mode, time);
}
...
{
	hal_scrSetBackLightValue(ucValue);
}
...
{
	return hal_scrSetAttrib(attr, value);
}
...
{
	hal_scrGotoxyPix(pixel_X, pixel_Y);
}
...
{
	hal_scrGetxyPix(pixel_X, pixel_Y);
}
...
void scrPrint_lib(uint col, unsigned char row, unsigned char mode, const char *str, ...)
{
	...
	hal_scrPrint(col, row, mode, "%s", rP);
...
void scrPrint_xy(uint col, uint row, unsigned char mode, const char *str, ...)
{
	...
	hal_scrPrintxy(col, row, mode, "%s", rP);
...
void scrPrint_Ex(uint col, unsigned char row, unsigned char mode, const char *chstr, const char *enstr)
{
	...
	hal_scrPrintEx(col, row, mode, chstr, enstr);
...
{
	hal_scrTextOut(pixel_X, pixel_Y, str);
}
...
{
	return hal_scrDrawDot(x, y, Mode);
}
...
{
	return hal_scrGetPixel(x, y, picolor);
}
...
{
	return hal_scrDrawStraightLine(sx, sy, ex, ey, Mode);
}
...
{
	hal_scrDrawRect(left, top, right, bottom, Mode);
}
...
{
	hal_scrDrawRectColor(left, top, right, bottom, color);
}
...
{
	return hal_scrDrawRectBlock(left, top, right, bottom, color);
}
...
{
	return hal_scrWriteLogo(left, top, data);
}
...
{
	hal_scrGetLcdSize(width, height);
}
...
{
	return hal_scrEnumFont(Fonts, MaxFontNums);
}
...
{
	return hal_scrSelectFont(SingleCodeFont, MultiCodeFont);
}
...
{
	return hal_scrSetLanguage(value);
}
...
{
	return hal_scrPopDot(popDot, lenth);
}
...
{
	return hal_scrPushDot(pushDot, lenth);
}
...
{
	return hal_scrSetTurnDegree(uiDegree);
}
...
{
	return hal_scrFullScreenSet(value);
}
...
void scrTest(void)
{
	scrSetAttrib_lib(LCD_ATTR_FONTTYPE, 0);
	scrPrint_lib(0, 2, 0x00, "hello,你好,%d", g_ui8BackLightValue);
	hal_scrPrint(0, 3, 0x00, "hello,你好,%d", g_ui8BackLightValue);
}
