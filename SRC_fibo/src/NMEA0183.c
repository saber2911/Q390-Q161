...
gps_gsv gpgsv[NMEA0183_GSV_MAX_LEN];  //gps
gps_gsv glgsv[NMEA0183_GSV_MAX_LEN];  //格林奈斯
gps_gsv gbgsv[NMEA0183_GSV_MAX_LEN];  //北斗
	...
	if(0!=frame&&0!=pos)
	{
		...
		while('*'!=*fr&&t<5000)
		{
			...
			}else if(','==*fr) 
			{
				...
				if(index>0)
				{
					sysLOG(API_LOG_LEVEL_4,"count:%d, index=%d, buff:%s\n", count, index, buff);
					if(1==count)
					{   
					...
					else if(3==count)  //字段3  可见卫星个数
					{
						...
						while('*'!=*fr) 
						{
							...
							elv = (buff[2]-0x30)*10+buff[3]-0x30;  //获取仰角(0~90)
							azi = (buff[4]-0x30)*100+(buff[5]-0x30)*10+buff[6]-0x30;  //获取方位角(0~359)
							DBH = (buff[7]-0x30)*10+buff[8]-0x30;  //获取信噪比
							switch(*(frame+1))
							{
								sysLOG(API_LOG_LEVEL_4,,"*(frame+1)=0x%x\n", *(frame+1));
								case 'P': //GP
								{								
									update_stll_msg(&gpgsv[0], NMEA0183_GSV_MAX_LEN, prn, DBH, elv, azi); //将卫星信息保存到定义的全局gps_msg缓冲中									
									break;
								}
								case 'B': //GB
								{
									update_stll_msg(&gbgsv[0], NMEA0183_GSV_MAX_LEN, prn, DBH, elv, azi);
									break;
								}
								case 'L': //GL
								{
									update_stll_msg(&glgsv[0], NMEA0183_GSV_MAX_LEN, prn, DBH, elv, azi);
									break;
								}
								default: break;							
							}
...
/* 更新卫星信息 */
static void update_stll_msg(gps_gsv* gsvMsg, unsigned char len, unsigned char prn, unsigned char dbh, unsigned char evl, int azi)  
{
	if(0!=gsvMsg&&0!=len) 
	{
		while(len--)
	...
	while('*'!=*fr&&t<5000)
	{
		...
		}else if(','==*fr) 
		{
			...
			if(index>0)
			{
				...
				{
					set_gps_latitude_str(buff);
					pos->latitude = gps_nmea_position_parse(buff); 
				}else if(3==count)
				{
					pos->lat_direc = buff[0];//N or S
				...
				{
					set_gps_longitude_str(buff);
					pos->longitude = gps_nmea_position_parse(buff);
				}else if(5==count) 
				{
					pos->long_direc = buff[0];// E or W
...
double gps_nmea_position_parse(char *s)
{
	char *cursor;
...
{
	int gpgsv_num = 0;
	for(int i=0;i<NMEA0183_GSV_MAX_LEN;i++) 
	{
		if(gpgsv[i].enable)
		{
			gpgsv_num ++;
