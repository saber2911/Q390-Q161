...
GB2312FONTCURRENT g_stGB2312FontCurrent;//存放当前选择GB2312字库信息

int g_iFontType = FONTFS;//FONTFLASH-采用Unicode编码格式的字库，并放置于flash中;FONTFS-采用GB2312编码格式的字库，并放置于文件系统中



unsigned int  g_single_fontnums;		/* Record the number of single font. */
unsigned int  g_total_fontnums;			/* Record the number of all fonts.	 */

ST_FONT g_enum_font[MAX_FONT_NUMS*2];

ST_FONT_TABLE g_stSingleFontLibTable[MAX_FONT_NUMS]; /* Record all sub-singlefontlib attribute/info.*/
ST_FONT_TABLE g_stMultiFontLibTable[MAX_FONT_NUMS];  /* Record all sub-multifontlib attribute/info. */

struct ExternFontLibAttr FontAttr;


/*
*Function:		hal_fontStrToLong
*Description:	字符串转long整形
*Input:			*str:字符串指针
*Output:		NULL
*Hardware:
*Return:		转换后的数值
*Others:
*/
unsigned long hal_fontStrToLong(unsigned char *str)
{
	unsigned long tmp = 0;
...
/*
*Function:		hal_fontGetFontType
*Description:	读取字库存储方式
*Input:			NULL
*Output:		NULL
*Hardware:
*Return:		FONTFLASH-采用Unicode编码格式的字库，并放置于flash中;FONTFS-采用GB2312编码格式的字库，并放置于文件系统中
*Others:
*/
...
/*
*Function:		hal_fontSetFontType
*Description:	设置字库存储方式
*Input:			FontType:FONTFLASH,FONTFS
*Output:		NULL
*Hardware:
...
int hal_fontSetFontType(unsigned char FontType)
{
	...
	if(FontType != FONTFLASH && FontType != FONTFS)
	{
		iRet = -1;
		goto exit;
	}
	
	g_iFontType = FontType;
	iRet = 0;

exit:
	
	sysLOG(FONT_LOG_LEVEL_5, "FontType=%d,iRet=%d\r\n", FontType, iRet);
	return iRet;
...
/**
 * @Function    : CalcBlankSize   
 * @Description : Calcute the unicode - blank area size.
 * @Input       : @unicode  -  .   
 *              : @len       -  .
 * @Output      : @nFontSize -  .
 * @Return      : return the black-size .
 *              : retrun -1. 
 * @Others      : Null.       
 */
...
/*
*Function:		hal_fontGetDot
*Description:	取字模
*Input:			reverse:是否翻转.0-正常显示，1-反显
*				*CurFont：当前所选字体ST_CUR_FONT指针;*str:所需要显示的字符指针;
*Output:		*dot_buf:取到的字模;*width:字模宽;*heigh:字模高
*Hardware:
*Return:		<=0:失败;其他值(1 or 2):encode width
*Others:
*/
int hal_fontGetDot(IN int reverse,ST_CUR_FONT *CurFont,IN unsigned char const *str,OUT char *dot_buf,OUT unsigned int *width,OUT unsigned int *heigh)
{
	...
	int  blk_size = 0, offset = 0;
	unsigned int buf_size = 0;                        /* Record dot_buf size */
	unsigned int i;
	int encode_width = 0;
	unsigned char *str_tmp = 0;
	
	int offsettmp = 0;
	int areaindex = 0;
	int Fontoffsettmp = 0;
	int sizeofonefont = 72;//一个汉字或ascii的像素占用多少个字节
	int sizeofonearea = 94;//一个区内有多少汉字 
	unsigned short GBcodetmp = 0;
	
	/*char buf[32] = { 0x00,0x00,0x00,0x00,0x78,0x1E,0x30,0x0C,0x18,0x18,0x0C,0x30,0x06,0x60,0x03,0xC0,
					 0x01,0x80,0x01,0x80,0x0F,0xF0,0x01,0x80,0x01,0x80,0x03,0xC0,0x00,0x00,0x00,0x00 };*///人民币符号内码
	unsigned char rmb16[32] = { 0x00,0x00,0x00,0x00,0x10,0x10,0x08,0x20,0x04,0x40,0x02,0x80,0x01,0x00,0x01,0x00,
	0x01,0x00,0x0F,0xE0,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x00,0x00};//16字体人民币符号内码

	unsigned char ydb16[32] = { 0x00,0x00,0x0F,0xF8,0x1F,0xF0,0x01,0xC0,0x0F,0xF0,0x1F,0xF0,0x00,0xC0,0x0F,0x80,
	0x0F,0x00,0x0E,0x00,0x07,0x00,0x03,0x80,0x01,0x80,0x01,0xC0,0x00,0xE0,0x00,0x00};//16字体印度币符号内码

	unsigned char rmb24[72] = {
		0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x3F,0xE1,0xFC,0x07,
		0x00,0x60,0x03,0x80,0xC0,0x01,0xC1,0xC0,0x01,0xE3,0x80,0x00,0xF7,0x00,0x00,0x7E,
		0x00,0x00,0x3C,0x00,0x00,0x3C,0x00,0x07,0xFF,0xE0,0x00,0x3C,0x00,0x00,0x3C,0x00,
		0x00,0x3C,0x00,0x00,0x3C,0x00,0x00,0x3C,0x00,0x00,0x3C,0x00,0x01,0xFF,0x80,0x00,
		0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};//24字体人民币符号内码

	unsigned char ydb24[72] = {
	0x00,0x00,0x00,0x00,0x00,0x00,0x03,0xFF,0xE0,0x07,0xFF,0xC0,0x07,0xFF,0xC0,0x00,
	0x1E,0x00,0x03,0xFF,0xE0,0x07,0xFF,0xC0,0x07,0xEE,0x80,0x00,0x0E,0x00,0x00,0x3C,
	0x00,0x03,0xF8,0x00,0x03,0xF0,0x00,0x03,0xC0,0x00,0x01,0xE0,0x00,0x01,0xE0,0x00,
	0x00,0xF0,0x00,0x00,0x78,0x00,0x00,0x3C,0x00,0x00,0x1E,0x00,0x00,0x1E,0x00,0x00,
	0x0F,0x00,0x00,0x00,0x00,0x00,0x00,0x00};//24字体印度币符号内码

	str_tmp = str;
	/* Check the str is assic code or other code; AND check current extern font is exit or not. */
	if(*str_tmp > 0x00 && CurFont->multi_dot_addr != NULL)//FontAttr.lcd_cur_font.multi_dot_addr != NULL)
	{
		encode_width = 2;		 
		*width = CurFont->multi_code_font.Width;//MultiCodeFont.Width;
		*heigh = CurFont->multi_code_font.Height;
        sysLOG(FONT_LOG_LEVEL_5, "*width=%d, *heigh=%d\r\n", *width, *heigh);
        if(*width % 8 == 0)
        {
            buf_size = (*width)*(*heigh)/8;
        }
        else
        {
            buf_size = ((*width/8)+1)*(*heigh);
        }
		unicode = (unsigned short)((unsigned short)(*str_tmp << 8) + *(str_tmp + 1));

		sysLOG(FONT_LOG_LEVEL_5, "*str=%x, *(str+1)=%x, unicode:%x\r\n", *str, *(str+1), unicode);

		if (unicode == 65509)//65509为中文人民币码
		{
			if(buf_size == 72)
			{
				memcpy((unsigned char*)dot_buf, rmb24, buf_size);
			}
			else if(buf_size == 32)
			{
				memcpy((unsigned char*)dot_buf, rmb16, buf_size);
			}
			else
			{
			}
		}
		else if (unicode == 8377)
		{
			if(buf_size == 72)
			{
				memcpy((unsigned char*)dot_buf, ydb24, buf_size);
			}
			else if(buf_size == 32)
			{
				memcpy((unsigned char*)dot_buf, ydb16, buf_size);
			}
			else
			{
			}
		...
		else
		{
			...
			GBcodetmp = hal_fontUnicode2GBcode(unicode);
			
			switch(FontAttr.lcd_cur_font.multi_code_font.Height){
			case 12:
				sizeofonefont = 2*12;	
			break;
			case 16:
		...
		areaindex = ((GBcodetmp&0xFF00)>>8) - 0xA1;
		offsettmp = (GBcodetmp&0x00FF) - 0xA1;
		sysLOG(FONT_LOG_LEVEL_5, "unicode=0x%x,GBcodetmp=0x%x,areaindex=%d,offsettmp=%d\r\n", unicode, GBcodetmp, areaindex, offsettmp);
		Fontoffsettmp = sizeofonefont*(areaindex*sizeofonearea + offsettmp);
		if(FontAttr.lcd_cur_font.multi_dot_addr != NULL)
		{
			hal_fileReadPro(FontAttr.lcd_cur_font.multi_dot_addr, Fontoffsettmp+FontAttr.lcd_cur_font.multi_dot_addroffset, dot_buf, sizeofonefont);
		}
		else
		{
			
			memset(dot_buf, 0xFF, sizeofonefont);
			
		}
		}
		if (reverse) 
        {
	        DotBufReverse(dot_buf,buf_size);
	    }	
	}
	else
    {                                               /* The char is singled or single width. */
		encode_width = 1;	
		*width = CurFont->single_code_font.Width;//>SingleCodeFont.Width;
		*heigh = CurFont->single_code_font.Height;//>SingleCodeFont.Height;
		
		switch(CurFont->single_code_font.Height)//>SingleCodeFont.Height)
		{
		case 8 : buf_size = 8 ; break;
		case 16: buf_size = 16; break;
		case 24: buf_size = 48; break;
		case 12: buf_size = 12; break;
		default: break;
        //case 16: buf_size = 16; break;
		//case 24: buf_size = 48; break;
		//case 32: buf_size = 16; break;
		//default: break;
		}
		str_tmp ++;
		if(*str_tmp < 0x20 )
        {
			memset(dot_buf,(unsigned char)0xff,buf_size);
		}
		else
        {
			i = (*str_tmp-0x20)*buf_size;
			if(i >= CurFont->single_font_lib_len)//>SingleFontLibLen)//FontAttr.lcd_cur_font.single_font_lib_len)
            {
					memset(dot_buf,(unsigned char)0xff,buf_size);
					return 0;
			}
					
			memcpy(dot_buf,(unsigned char*)(CurFont->single_dot_addr + i),buf_size);
...
/*
*Function:		hal_fontInit
*Description:	初始化字库
*Input:			fontlib_exit_flag:确定外部字库是否存在;0-不存在;1-存在;*fontlib_addr:字库地址指针
*Output:		NULL
*Hardware:
*Return:		NULL
*Others:
...
void hal_fontInit(FONTLIB_TYPE fontlib_exit_flag,unsigned char *fontlib_addr)
{
	...
	uint32 sub_fontnum = 0;
	memset(tmp_buf, 0, sizeof(tmp_buf));
	
	/* check extern font-lib is exit or not. */
	if(EXTERN_FONTLIB == fontlib_exit_flag){/* If the extern-fontlib is exited  */

		FontAttr.exfontlib_addr = fontlib_addr;
		hal_fileReadPro(FontAttr.exfontlib_addr, 0, tmp_buf, 16);

		FontAttr.fontlib_ver = tmp_buf[8];
		sub_fontnum = tmp_buf[9];
		memset(&FontAttr.lcd_cur_font,0,sizeof(FontAttr.lcd_cur_font));
		sysLOG(FONT_LOG_LEVEL_5, "sub_fontnum=%d\r\n", sub_fontnum);

		if((sub_fontnum > MAX_FONT_NUMS)||(memcmp(tmp_buf,"VST-FONT",8) != 0)){
			sub_fontnum = 0;
			sysLOG(FONT_LOG_LEVEL_4, "<WARN>FontAttr.sub_font_nums=%d\r\n", sub_fontnum);
		}

		if(sub_fontnum){
			for(i=FontAttr.sub_font_nums; i<(FontAttr.sub_font_nums+sub_fontnum); i++){
				hal_fileReadPro(FontAttr.exfontlib_addr, 16 + (i-FontAttr.sub_font_nums)*16, tmp_buf, 16);
				g_stMultiFontLibTable[i].st_font.CharSet	= tmp_buf[0];     //sub-fontlib encode type.
				g_stMultiFontLibTable[i].st_font.Width		= tmp_buf[1];
				g_stMultiFontLibTable[i].st_font.Height		= tmp_buf[2];
				g_stMultiFontLibTable[i].st_font.Bold		= tmp_buf[3];
				g_stMultiFontLibTable[i].st_font.Italic		= tmp_buf[4];
				g_stMultiFontLibTable[i].st_font.EncodeSet	= (CHAR_TYPE)tmp_buf[5];     //char display attribute set(eg:signal/multi/arabic...)
				g_stMultiFontLibTable[i].sub_fontlib_addr	= FontAttr.exfontlib_addr;
				g_stMultiFontLibTable[i].sub_fontlib_addroffset = hal_fontStrToLong(tmp_buf+8);
				g_stMultiFontLibTable[i].fontlib_len		= hal_fontStrToLong(tmp_buf+12);
				sysLOG(FONT_LOG_LEVEL_5, "i=%d,FontAttr.exfontlib_addr:%s,sub_fontlib_addroffset=%d,tmp_buf[0]:%d,%d,%d,%d,%d\r\n", i, FontAttr.exfontlib_addr, g_stMultiFontLibTable[i].sub_fontlib_addroffset, tmp_buf[0], tmp_buf[1], tmp_buf[2], tmp_buf[3], tmp_buf[4]);
			};
		}
		

		memset(&FontAttr.lcd_cur_font,0,sizeof(FontAttr.lcd_cur_font));
		sysLOG(FONT_LOG_LEVEL_5, "FontAttr.sub_font_nums=%d\r\n", FontAttr.sub_font_nums);
		if(sub_fontnum > 0)
		{
			FontAttr.lcd_cur_font.multi_dot_addr       = g_stMultiFontLibTable[0].sub_fontlib_addr;
			FontAttr.lcd_cur_font.multi_code_font      = g_stMultiFontLibTable[0].st_font;
			FontAttr.lcd_cur_font.multi_font_lib_len   = g_stMultiFontLibTable[0].fontlib_len;
			FontAttr.sub_font_nums += sub_fontnum;
		}
		else
		{
			FontAttr.lcd_cur_font.multi_dot_addr       = NULL;
			//FontAttr.lcd_cur_font.multi_code_font      = ;
			FontAttr.lcd_cur_font.multi_font_lib_len   = 0;
		}
	}
	else{
		FontAttr.lcd_cur_font.multi_dot_addr       = NULL;
	...
	/* init internal single code array-dot fontlib.*/	
	g_stSingleFontLibTable[0].sub_fontlib_addr		= (unsigned char*)DefASCFont8X16;
	g_stSingleFontLibTable[0].st_font.CharSet		= SINGLE_CHAR;
	g_stSingleFontLibTable[0].fontlib_len			= 96*16;
	g_stSingleFontLibTable[0].st_font.EncodeSet		= ENCODE_WEST;
	g_stSingleFontLibTable[0].st_font.Bold			= 0;   
	g_stSingleFontLibTable[0].st_font.Italic		= 0;
	g_stSingleFontLibTable[0].st_font.Width			= 8;
	g_stSingleFontLibTable[0].st_font.Height		= 16;
	
	g_stSingleFontLibTable[1].sub_fontlib_addr		= (unsigned char*)DefASCFont12X24;
	g_stSingleFontLibTable[1].st_font.CharSet		= SINGLE_CHAR;
	g_stSingleFontLibTable[1].fontlib_len			= 96*48;
	g_stSingleFontLibTable[1].st_font.EncodeSet		= ENCODE_WEST;
	g_stSingleFontLibTable[1].st_font.Bold			= 0;
	g_stSingleFontLibTable[1].st_font.Italic		= 0;
	g_stSingleFontLibTable[1].st_font.Width			= 12;
	g_stSingleFontLibTable[1].st_font.Height		= 24;

	g_stSingleFontLibTable[2].sub_fontlib_addr		= (unsigned char*)DefASCFont6X12;
	g_stSingleFontLibTable[2].st_font.CharSet		= SINGLE_CHAR;
	g_stSingleFontLibTable[2].fontlib_len			= 96*16;
	g_stSingleFontLibTable[2].st_font.EncodeSet		= ENCODE_WEST;
	g_stSingleFontLibTable[2].st_font.Bold			= 0;   
	g_stSingleFontLibTable[2].st_font.Italic		= 0;
	g_stSingleFontLibTable[2].st_font.Width			= 6;
	g_stSingleFontLibTable[2].st_font.Height		= 12;

	g_single_fontnums = 3;				/* Record the number of single font libs. */
	
	/* # Set default font --- Init the font-lib's attribute which current screen using.  */
	//
	//  Please Add FindFont() and SetDefaultFont() function here.
	//
	FontAttr.lcd_cur_font.single_dot_addr      = g_stSingleFontLibTable[0].sub_fontlib_addr;
	FontAttr.lcd_cur_font.single_code_font     = g_stSingleFontLibTable[0].st_font;
	FontAttr.lcd_cur_font.single_font_lib_len  = g_stSingleFontLibTable[0].fontlib_len;

}


/*
*Function:		hal_fontLoad
*Description:	加载字库
*Input:			NULL
*Output:		NULL
*Hardware:
...
int hal_fontLoad(void)
{
	...
	FontAttr.sub_font_nums = 0;              /* ExternSubFontLibNum is a sataic-argument, init it here*/
	g_total_fontnums  = 0;

	memset(g_stSingleFontLibTable, 0, sizeof(g_stSingleFontLibTable));
	memset(g_stMultiFontLibTable, 0, sizeof(g_stMultiFontLibTable));

	hal_fontInit(EXTERN_FONTLIB, GB2312FONT12X12FILENAME);
	hal_fontInit(EXTERN_FONTLIB, GB2312FONT16X16FILENAME);
	hal_fontInit(EXTERN_FONTLIB, GB2312FONT24X24FILENAME);

//	hal_fontInit(EXTERN_FONTLIB, GB2312FONTFILENAME);
...
/*
*Function:		hal_fontSelect
*Description:	选择字体
*Input:			*SingleCodeFont:单模字体指针;*MultiCodeFont:双模字体指针
*Output:		NULL
*Hardware:
*Return:		0-成功；other-失败
*Others:
...
int hal_fontSelect(ST_FONT *SingleCodeFont, ST_FONT *MultiCodeFont)
{
	...
	{
		if((SingleCodeFont->Width > MAX_FONT_WIDTH)||(SingleCodeFont->Height > MAX_FONT_HEIGHT))
		{
			return SINGLE_CODE_FONT_OVER;
		}
		
		for(i = 0; i < g_total_fontnums;i++)
		{
			if(!memcmp(&g_enum_font[i],SingleCodeFont,sizeof(ST_FONT))) break;
		}
		sysLOG(FONT_LOG_LEVEL_5, "SingleCodeFont:i=%d, g_total_fontnums=%d\r\n", i, g_total_fontnums);
		if(i < g_total_fontnums)
		{
			
			switch(SingleCodeFont->Height)
			{
				//case 8 : {g_single_fontsize = 0; g_line_height = 8; st_lcd_info.ppc = 6; st_lcd_info.ppl = 8; break; }
				case 12: //{g_single_fontsize = 0; g_line_height = 16; st_lcd_info.ppc = 8; st_lcd_info.ppl = 16; break; }
				case 16: //{g_single_fontsize = 1; g_line_height = 24; st_lcd_info.ppc = 12; st_lcd_info.ppl = 24; break;}
				case 24: //{g_single_fontsize = 2; g_line_height = 32; st_lcd_info.ppc = 16; st_lcd_info.ppl = 32; break;}
				break;
				default: return SINGLE_CODE_FONT_NON; break;
			}
			memcpy(&FontAttr.lcd_cur_font.single_code_font,SingleCodeFont,sizeof(ST_FONT));
			FontAttr.lcd_cur_font.single_dot_addr = g_stSingleFontLibTable[i-FontAttr.sub_font_nums].sub_fontlib_addr;
			FontAttr.lcd_cur_font.single_font_lib_len = g_stSingleFontLibTable[i-FontAttr.sub_font_nums].fontlib_len;
			sysLOG(FONT_LOG_LEVEL_5, "SingleCodeFont:sub_font_nums:%d, width=%d, height=%d\r\n", FontAttr.sub_font_nums, FontAttr.lcd_cur_font.single_code_font.Width, FontAttr.lcd_cur_font.single_code_font.Height);
		}
		else
		{
			return SINGLE_CODE_FONT_NON;
		}
	}
	...
	{
		if((MultiCodeFont->Width > MAX_FONT_WIDTH)||(MultiCodeFont->Height > MAX_FONT_HEIGHT))
		{
			return MULTICODEFONT_CODE_FONT_OVER;
		}

		for(i = 0; i < g_total_fontnums;i++)
		{
			if(!memcmp(&g_enum_font[i],MultiCodeFont,sizeof(ST_FONT))) break;
		}
		sysLOG(FONT_LOG_LEVEL_5, "MultiCodeFont:i=%d, g_total_fontnums=%d\r\n", i, g_total_fontnums);
		if(i < g_total_fontnums){
			switch(MultiCodeFont->Height)
			{
			case 12: //g_multi_fontsize = 0; break;
			case 16: //g_multi_fontsize = 1; break;			/* Now there is 16X16 fontsize only. */
			case 24: //g_multi_fontsize = 2; break;
			break;
			//case 32: g_multi_fontsize = 3; break;
			default: return MULTICODEFONT_CODE_FONT_NON; break;
			}
			memcpy(&FontAttr.lcd_cur_font.multi_code_font,&g_stMultiFontLibTable[i],sizeof(ST_FONT));
			FontAttr.lcd_cur_font.multi_dot_addr = g_stMultiFontLibTable[i].sub_fontlib_addr;
			FontAttr.lcd_cur_font.multi_dot_addroffset = g_stMultiFontLibTable[i].sub_fontlib_addroffset;
			FontAttr.lcd_cur_font.multi_font_lib_len = g_stMultiFontLibTable[i].fontlib_len;

			sysLOG(FONT_LOG_LEVEL_5, "MultiCodeFont:i:%d, width=%d, height=%d\r\n", i, FontAttr.lcd_cur_font.multi_code_font.Width, FontAttr.lcd_cur_font.multi_code_font.Height);
		}
		else
		{
			FontAttr.lcd_cur_font.multi_dot_addr = NULL;
			FontAttr.lcd_cur_font.multi_dot_addroffset = 0;
			FontAttr.lcd_cur_font.multi_font_lib_len = 0;
			return MULTICODEFONT_CODE_FONT_NON;
		}
	}
...
/*
*Function:		hal_fontEnum
*Description:	枚举字体
*Input:			MaxFontNums:最多可以枚举字体个数
*Output:		*Fonts:枚举到的字体指针;
*Hardware:
*Return:		<0:失败;>=0:枚举到的字体个数
*Others:
*/
int hal_fontEnum(ST_FONT *Fonts,int MaxFontNums)
{
	...
	if (MaxFontNums < 0)
	{
		return ERROR_FONT_PARAM;
	}
	if (MaxFontNums >= (int)FontAttr.sub_font_nums + g_single_fontnums) 
	{
		g_total_fontnums = FontAttr.sub_font_nums + g_single_fontnums;
	}
	else 
	{
		return FONTS_BUF_OVERFLOW;
	}
	sysLOG(FONT_LOG_LEVEL_5, "g_total_fontnums:%d,FontAttr.sub_font_nums=%d,g_single_fontnums=%d\r\n", g_total_fontnums, FontAttr.sub_font_nums, g_single_fontnums);

	for (i = 0, j = 0; i < FontAttr.sub_font_nums; i++, j++) 
	{
		memcpy(&Fonts[i], &g_stMultiFontLibTable[j], sizeof(ST_FONT));
	}

	for (i = FontAttr.sub_font_nums, j = 0; j < g_single_fontnums; i++, j++) 
	{
		memcpy(&Fonts[i], &g_stSingleFontLibTable[j], sizeof(ST_FONT));
	}

	//memcpy(&g_enum_font[0], &Fonts[0], g_total_fontnums * sizeof(ST_FONT));

	return g_total_fontnums;
}
...
/*
*Function:		hal_fontUnicode2GBcode
*Description:	字符，Unicode转GB2312
*Input:			iUnicode:unicode编码的字符值
*Output:		NULL
*Hardware:
*Return:		0-失败;other-转换后的GB2312编码的数值
*Others:
*/
unsigned short hal_fontUnicode2GBcode(unsigned short iUnicode)
{
			...
			- 1; n > 0; n >>= 1, ++j)
	{
		...
		{
			sysLOG(FONT_LOG_LEVEL_5, "\nUnicode_GB2312[%d][1]=[%x]\n", i, Unicode_GB2312[i][1]);
...
/*
*Function:		hal_fontGBcode2Unicode
*Description:	字符，GB2312转Unicode
*Input:			iGBcode:gb2312编码的字符值
*Output:		NULL
*Hardware:
*Return:		0-失败;other-转换后的unicode编码的数值
*Others:
*/
unsigned short hal_fontGBcode2Unicode(unsigned short iGBcode)
{
	...
	int k = 0;
	int i = 0;
	unsigned char strGBcode[] = {(iGBcode&0xFF00)>>8, iGBcode&0x00FF};
    unsigned char buftmp[8];
    unsigned short int tmp = 0;
	
	memset(buftmp, 0, sizeof(buftmp));

	sysLOG(FONT_LOG_LEVEL_5, "len:%d\n", len);
	while (i < len)
	{
		if ((unsigned char) strGBcode[i] < 0xa1)
		{
		    buftmp[j++] = 0x0;
			buftmp[j++] = strGBcode[i++];			
		}
		else
		{
			tmp = strGBcode[i] & 0xff;
			tmp = ((tmp << 8) & 0xff00) | (strGBcode[i + 1] & 0xff);
			i += 2;
					...
					- 1; k++)
			{
				...
				{
					buftmp[j++] = (Unicode_GB2312[k][0] >> 8) & 0xff;
					buftmp[j++] = Unicode_GB2312[k][0] & 0xff;
					break;
				}
			}
			/*没找到*/
			...
			{
				sysLOG(FONT_LOG_LEVEL_4, "not find\n");
				return 0;
			}
		}
	}
	buftmp[j] = '\0';
	return (buftmp[0]<<8 | buftmp[1]);

}


/*
*Function:		hal_fontStrUnicode2GB
*Description:	转换Unicode字符串到GB2312
*Input:			*strSourcer:待转码的字符串，n:unicode字符串长度
*Output:		*strDest:转换后的字符串
*Hardware:
*Return:		>=0:返回汉字数；<0:失败
*Others:
*/
int hal_fontStrUnicode2GB(const unsigned char *strSourcer, char *strDest, int n)
{
	char cTmp;
	...
	for (i = 0, pSrc = strSourcer, pDest = strDest; n > 0; n -= 2, pSrc += 2, ++i, ++pDest)
	{
		...
		tmphz = hal_fontUnicode2GBcode(hz);
...
/*
*Function:		hal_fontStrGB2Unicode
*Description:	转换GB2312字符串到Unicode
*Input:			*str:待转码的字符串，n:unicode字符串长度
*Output:		*dst:转换后的字符串
*Hardware:
*Return:		>=0:返回汉字数；<0:失败
*Others:
*/
int hal_fontStrGB2Unicode(const char *str, unsigned char *dst, int n)
{
